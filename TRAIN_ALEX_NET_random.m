...
outputDir = ['trained_networks_scratch_participant_subtract_mean_random' num2str(pp)];
...
idx = randperm(numel(trainImds.Labels));

% Shuffle labels (and images if applicable)
trainImds.Labels = trainImds.Labels(idx);
...
augVal = augmentedImageDatastore( ...
    ...
    save(fullfile(outputDir, sprintf('alexnet_scratch_random_I%d.mat', i)), 'netScratch');
end
end
